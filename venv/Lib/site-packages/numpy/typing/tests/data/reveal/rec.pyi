...
assert_type(
    np.recarray(
        shape=(10, 5), dtype=[("f8", np.float64), ("i8", np.int64)], strides=(5, 5),
    ),
...
assert_type(
    np.rec.fromarrays(AR_LIST, dtype=np.int64), np.recarray[Any, np.dtype[Any]],
)
assert_type(
...
assert_type(
    np.rec.fromfile("test_file.txt", dtype=[("i8", np.int64), ("f8", np.float64)],),
    np.recarray[Any, np.dtype[Any]],
)
...
assert_type(
    np.rec.array(None, dtype=np.float64, shape=(10, 3),),
    np.recarray[Any, np.dtype[Any]],
)

assert_type(
    np.rec.array(
        None, formats=[np.int64, np.float64], names=["i8", "f8"], shape=(10, 3),
    ),
    np.recarray[Any, np.dtype[np.record]],
)

assert_type(
    np.rec.array(file_obj, dtype=np.float64), np.recarray[Any, np.dtype[Any]],
)
